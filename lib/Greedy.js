var GreedyMesh = (function() {
  ...
  //var mask = new Int32Array(4096); // why 4096? chunk size?
	var mask = new Int32Array(320);
      ...
      for (x[dim] = -1; x[dim] < dims[dim];) {
        ...
        for (x[v] = 0; x[v] < dims[v]; x[v]++) {
          ...
          for (x[u] = 0; x[u] < dims[u]; x[u]++, voxelIdx++) {
            ...
            var a = x[dim] >= 0 ? getVoxel(x[0], x[1], x[2]) : 0;
            var b = x[dim] < dims[dim] - 1 ? getVoxel(x[0] + quad[0], x[1] + quad[1], x[2] + quad[2]) : 0;
        ...
        for (j = 0; j < dims[v]; j++) {
          ...
          for (i = 0; i < dims[u];) {
            ...
            if (!!color) {
              ...
              vertices.push([x[0] + du[0] + dv[0], x[1] + du[1] + dv[1], x[2] +
                du[2] + dv[2]
              ]);
              vertices.push([x[0] + dv[0], x[1] + dv[1], x[2] + dv[2]]);
              faces.push([vertex_count, vertex_count + 1, vertex_count + 2,
                vertex_count + 3, color
              ]);
...
GreedyMesh.TestData = (function createTestData() {
  ...
  for (var i = 1, c = 0; i <= 16; i <<= 1, ++c) {
    ...
    result[i + 'x' + i + 'x' + i] = makeVoxels([0, 0, 0], [i, i, i], function() {
      return colorTab[c];
    });
  }

  result['Sphere'] = makeVoxels([-16, -16, -16], [16, 16, 16], function(i, j,
    k) {
  ...
  result['16 Color Noise'] = makeVoxels([0, 0, 0], [16, 16, 16], function(i, j, k) {
    ...
    return Math.random() < 0.1 ? colorTab[Math.floor(Math.random() * colorTab
      .length)] : 0;
  });

  result['Hilly Terrain'] = makeVoxels([0, 0, 0], [32, 32, 32], function(i, j,
    k) {
