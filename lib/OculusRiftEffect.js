...
THREE.OculusRiftEffect = function ( renderer, options ) {
	...
	this.render = function ( scene, camera, pitch ) {
		...
		pCamera.matrix.copy(camera.matrix).multiply(pitch.matrix).multiply(left.tranform);
		...
		pCamera.matrix.copy(camera.matrix).multiply(pitch.matrix).multiply(right.tranform);
		pCamera.matrixWorldNeedsUpdate = true;
