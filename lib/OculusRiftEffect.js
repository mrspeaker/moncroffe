...
THREE.OculusRiftEffect = function ( renderer, options ) {
	...
	this.render = function ( scene, cameraMatrix ) {
		...
		pCamera.matrix.copy(cameraMatrix).multiply(left.tranform);
		...
		pCamera.matrix.copy(cameraMatrix).multiply(right.tranform);
		pCamera.matrixWorldNeedsUpdate = true;
